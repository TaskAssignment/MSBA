...
//		fieldValue = fieldValue.replaceAll("(?s)(?<!\\S).(?!\\S)", " "); //to replace any single character by space.
	...
	//------------------------------------------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------------------------------------------------------------------
	//This method extracts tags that are separated by ;; and removes [] from the sides (the tags will be separated by " " at the end):
	public static String extractspaceSeparatedTags(String tags){
		tags = tags.substring(1, tags.length()-1); //: removing [] from the two sides.
		String[] tags_SA = tags.split(";;");
		String result = "";
		for (int i=0; i<tags_SA.length; i++)
			if (tags_SA[i].length() > 2)
				result = concatTwoStringsWithDelimiter(result, tags_SA[i], " ");
		return result;
	}
	//------------------------------------------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------------------------------------------------------------------
