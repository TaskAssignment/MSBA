...
public class Evidence {
	...
	public int[] nonBA__or_subProj_virtualSeqNum; //If the evidence is bug assignment, then these values are Constants.THIS_IS_NOT__NON_B_A_EVIDENCE. Otherwise the sequence number of the bugAssignment type 0 to 4 that is at the same time (or the last one before this evidence).
	...
	public Evidence(Date date, int seqNum, int[] virtualSeqNum, int freq, int totalNumberOfWordsInThisEvidence, BTOption3_TF option3_TF){
		...
		this.nonBA__or_subProj_virtualSeqNum = new int[Constants.NUMBER_OF_ASSIGNEE_TYPES];
		for (int i=ASSIGNMENT_TYPE_TO_TRIAGE.T1_AUTHOR.ordinal(); i<=ASSIGNMENT_TYPE_TO_TRIAGE.T5_ALL_TYPES.ordinal(); i++)
			this.nonBA__or_subProj_virtualSeqNum[i] = virtualSeqNum[i];
		this.freq = freq;
